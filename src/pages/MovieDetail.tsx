...
export default function MovieDetail({ type }: Props) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center pt-16">
      <div className={`w-12 h-12 border-2 border-t-transparent rounded-full animate-spin ${isMovie ? 'border-blue-500' : 'border-green-500'}`} />
    </div>
  )
  if (!item) return (
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center pt-16">
      <p className="text-gray-400">{lang === 'zh' ? '未找到内容' : 'Not found.'}</p>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] pb-20">
      ...
      <div className="relative h-[60vh] overflow-hidden">
        {item.backdrop_path
          ? <img src={getImageUrl(item.backdrop_path, 'original')} alt={title} className="absolute inset-0 w-full h-full object-cover" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-40 relative">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-shrink-0">
            <div className="w-48 md:w-56 rounded-2xl overflow-hidden shadow-2xl">
              {item.poster_path
                ...
                : <div className="aspect-[2/3] bg-gray-800 flex items-center justify-center text-5xl">🎬</div>
              }
            </div>
          </div>
          ...
          <div className="flex-1 pt-4 md:pt-20">
            <p className={`section-label ${accentColor} mb-2`}>{T(isMovie ? t.detail.movie : t.detail.tvShow)}</p>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-5 text-sm">
              ...
              {runtime && <span className="text-gray-400">{formatRuntime(runtime)}</span>}
              {!isMovie && (item as TvShow).number_of_seasons && (
                <span className="text-gray-400">{(item as TvShow).number_of_seasons} {T(t.detail.seasons)}</span>
              )}
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <button onClick={() => play({ type, id: item.id, title })}
                className={`flex items-center gap-2 ${accentBg} text-white font-bold px-6 py-3 rounded-xl transition-all`}>
                <svg className="w-5 h-5 ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                {T(t.detail.watchNow)}
              </button>
              <button
                onClick={() => toggleWatchlist({ id: item.id, type, title, poster: item.poster_path, score: item.vote_average, year: year || null })}
        ...
        {item.cast && item.cast.length > 0 && (
          <section className="mt-12">
            <p className={`section-label ${accentColor} mb-2`}>{T(t.detail.castLabel)}</p>
            <h2 className="text-xl font-black text-white mb-6">{T(t.detail.cast)}</h2>
        ...
        {similar.length > 0 && (
          <section className="mt-12">
            <p className={`section-label ${accentColor} mb-2`}>{T(t.detail.moreLike)}</p>
            <h2 className="text-xl font-black text-white mb-6">{T(t.detail.similar)}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {similar.slice(0, 6).map(m => <MovieCard key={m.id} item={m} type={type} />)}
            </div>
          </section>
        )}
      </div>
